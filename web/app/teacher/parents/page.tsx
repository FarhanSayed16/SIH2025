...
export default function TeacherParentsPage() {
  ...
  useEffect(() => {
    ...
    if (teacherInstitutionId) {
      ...
      showToast('You must be assigned to an institution to manage parents', 'error');
      setIsLoading(false);
    }
  }, [isAuthenticated, user, router, accessToken, teacherInstitutionId, showToast]);
  ...
  const loadParents = useCallback(async () => {
    if (!teacherInstitutionId) return;
    ...
    try {
      ...
      // Fetch all parents from teacher's institution across multiple pages
  ...
  }, [showToast, accessToken, teacherInstitutionId]);
  ...
  const handleViewParent = (parentId: string) => {
    router.push(`/teacher/parents/${parentId}`);
  };

  const handleLinkChild = (parentId: string) => {
    router.push(`/teacher/parents/${parentId}/link-child`);
  };
  ...
  const handleCreateParent = async (e: React.FormEvent) => {
    ...
    if (!teacherInstitutionId) {
      showToast('You must be assigned to an institution to add parents', 'error');
      return;
    }
    ...
    try {
      ...
      const response = await authApi.register({
        ...
        role: 'parent',
        institutionId: teacherInstitutionId, // Automatically set to teacher's institution
      });
          ...
          <main className="flex-1 overflow-y-auto bg-gradient-to-br from-blue-50 via-white to-blue-50 p-6">
            <Card className="p-12 text-center">
              ...
              <p className="text-gray-600">
                You must be assigned to an institution to manage parents.
              </p>
            </Card>
          </main>
        </div>
